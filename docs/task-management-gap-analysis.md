...
| RBAC | Configurable resource/action roles, deny by default | **Partial.** Static roles plus per-user permission overrides; server checks are authoritative; `tasks.assign/review/approve/score/archive` are now separate keys and none reach an employee | Roles are not admin-managed; project/task-level responsibilities are still not evaluated alongside the organization role | Low | P1 | Persist custom roles once responsibilities exist | Permission matrix test per role × action |
...
| Recurring work | Daily/weekly/monthly templates generate dated task instances without duplication | **Implemented for HR calendar work.** The 51-task catalogue has per-template enablement and ownership; 44 calendar rules materialise dated tasks with deterministic IDs, working-day/month-end/quarter-end windows, 14-day restart catch-up and notifications. Seven event-driven rules stay manual so the clock cannot invent a hire/course/legal deadline | Public-holiday dates, admin-edited schedule overrides and versioned ownership/policy history still need the final HR operating cycle | Low | P1 | Import the confirmed cycle as schedule overrides without weakening the idempotent generator | Date-boundary, event-vs-calendar, duplicate retry and authenticated permission tests (present) |
| Workflow | Configurable states and guarded transitions | **Partial.** Department workflows are centralized in code; submission/review gates cannot be bypassed; for the person doing the work a stage is the result of an action and never a drag, so backwards and same-type moves are refused rather than silently allowed | No admin workflow editor, transition conditions, per-task-type workflow or workflow version | Medium | P1 | Persist versioned workflows and validate every transition server-side | Allowed/forbidden transition matrix |
...
| Delete/archive | Soft delete, archive, recycle bin, restore | **Partial.** `archivedAt/archivedBy/archiveReason` with archive and restore actions; archived work leaves every board, count, export and search but stays fetchable; the permanent purge is administrator-only and refuses anything not already archived | No recycle-bin view, no retention/purge schedule, and the audit trail behind it is still mutable | Medium | P1 | Add an archive view and a retention job on top of the existing timestamps | Archive → restore → purge integration test (present) plus immutable audit test |
...
1. immutable audit (soft deletion is done; the log behind it is still mutable);
2. ~~explicit resource permissions~~ — done: one key per authority, employee
   holds none of the commissioning side;
