...
|---|---|---|---|
| **الإيفينتات** | `event.event` + `event.track` + `event.registration` | training with a date — online and in person (see [EVENTS_SCHEDULE.md](EVENTS_SCHEDULE.md)) | lectures at a time, a room or a Zoom link, a register, an instructor |
...
own analysis tab and its own Allowed-apps tile, so somebody who may see the
...
|---|---|---|
| `event.event` | a course | `code`, `name`, `date_begin`, `date_end`, `stage_id`, `event_type`, `attendance_method`, `if_offline`, `headquarter`, `instructor_id`, `total_lectures_number`, `session_duration`, `seats_max`, `address_id` — plus any custom concept fields found by discovery. **Not** `seats_taken` (see below) |
| `event.track` | one lecture | `name`, `date`, `duration`, `event_id` (+ Zoom link fields for Today and the detail drawer only) |
| `event.registration` | one registration | grouped by `event_id`, `state` — never read row by row |
...
any missing variable. `POST /api/events/refresh` expires the Events caches (the last good
answer is kept as a fallback). `GET /api/events/diagnostics` (administrators) reports what
field discovery found on the live `event.event` / `event.track` — names, labels, types and
the `store` flag, never records or credentials.
...
The analysis tab in both apps (Events → **التحليل**; eLearning's first tab) opens on the current calendar
...
the period when it starts inside it. The analysis counts in-person events only
(`attendance_method = offline` in the Odoo domain); the Training Schedule, Today
and the detail drawer show online courses too, because the operations schedule
mixes both.
