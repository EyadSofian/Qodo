...
async function run(browser, months) {
  ...
  /* 9–13. Done history, board vs table, review queue. */
  {
    ...
    const picker = page.getByRole('group', { name: 'Month', exact: true }).first();
    ...
    check('month: no stepping past the newest work', await picker.getByRole('button', { name: 'Next month' }).isDisabled());
    ...
    check('table: open work dated this month is not in last month', !(await rows()).includes('QA race task renamed'));
    check('table: says how much open work is in other months', await visible(page.getByText(/open tasks dated in other months/)));
    ...
    check('board: open work dated this month is back', await visible(page.getByText('QA race task renamed')));
