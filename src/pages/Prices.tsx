/**
 * دليل الأسعار — what a seller may quote, while the customer is still on the
 * line.
 *
...
interface AdviceResponse {
  ok: boolean;
  configured: boolean;
  book: { version: number; effectiveFrom: string } | null;
  matches: Match[];
  advice: Advice | null;
  error: string;
}
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6">
      <header className="mb-5">
        <h1 className="text-xl font-bold text-ink">{ar ? 'دليل الأسعار' : 'Price advisor'}</h1>
        <p className="mt-1 text-[13.5px] leading-relaxed text-ink-muted">
          {ar
            ? 'السعر المعتمد للدورة، ومساحة التفاوض المسموحة، وهل السعر يحتاج موافقة مدير.'
            : 'The approved price, the room you have to negotiate in, and whether the number needs a manager.'}
        </p>
        {data?.book && (
          <p className="mt-1 text-[11.5px] text-ink-faint">
            {ar ? 'كتاب الأسعار' : 'Price book'} v{data.book.version} ·{' '}
            {ar ? 'ساري من' : 'in force since'} {data.book.effectiveFrom}
          </p>
        )}
      </header>
